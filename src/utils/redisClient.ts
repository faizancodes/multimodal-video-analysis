...
export async function cacheVideoTranscript(videoId: string, transcript: TranscriptItem[]): Promise<void> {
  try {
...
export async function getCachedVideoTranscript(videoId: string): Promise<TranscriptItem[] | null> {
  try {
    const key = getVideoTranscriptKey(videoId)
    ...
    logger.error(`Failed to retrieve cached transcript for video ${videoId}`, error)
...
export async function clearVideoTranscriptCache(videoId: string): Promise<void> {
  try {
    const key = getVideoTranscriptKey(videoId)
    await redisClient.del(key)
    logger.info(`Cleared cached transcript for video ${videoId}`)
  } catch (error) {
    logger.error(`Failed to clear cached transcript for video ${videoId}`, error)
  }
}
