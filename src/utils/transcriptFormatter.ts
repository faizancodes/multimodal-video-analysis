 ...
 */
export function formatTranscriptIntoSentences(transcript: TranscriptItem[]): FormattedSentence[] {
  ...
  function estimateTimestamp(fragment: TranscriptItem, charPosition: number, totalChars: number): number {
  ...
  function createSentence(text: string, startTime: number, endTime: number, lang: string): FormattedSentence {
    return {
      text: text.trim(),
      startTime,
      endTime,
  ...
  function findSentenceBoundaries(text: string): { position: number, char: string }[] {
    const boundaries = []
    ...
    while ((match = sentenceEnders.exec(text)) !== null) {
      ...
      const afterPunctuation = text.substring(match.index + match[0].length).trim()
  ...
  for (let i = 0; i < transcript.length; i++) {
    ...
    if (currentSentence && !currentSentence.endsWith(' ') && !trimmedText.startsWith(' ')) {
      currentSentence += ' '
    }
    ...
    if (sentenceBoundaries.length > 0) {
      ...
      for (const boundary of sentenceBoundaries) {
        const sentencePart = trimmedText.substring(lastBoundary, boundary.position)
        currentSentence += sentencePart

        // Calculate end time for this sentence
        const endTime = estimateTimestamp(item, boundary.position, trimmedText.length)

        // Create sentence if we have accumulated text
        if (currentSentence.trim()) {
          const sentence = createSentence(currentSentence, sentenceStartTime, endTime, item.lang || 'en')
      ...
      if (remainingText) {
        ...
        const startTime = estimateTimestamp(item, lastBoundary, trimmedText.length)
  ...
  if (currentSentence.trim() && hasStartedSentence) {
    ...
    const sentence = createSentence(currentSentence, sentenceStartTime, endTime, lastItem.lang || 'en')
    sentences.push(sentence)
  }
 ...
 */
export function formatTranscriptWithTimestamps(transcript: TranscriptItem[]): (FormattedSentence & { formattedStartTime: string, formattedEndTime: string })[] {
